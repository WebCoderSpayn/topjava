...
//        mealsTo.forEach(System.out::println);
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        meals.forEach(userMeal -> {
            ...
            Integer calories = days.getOrDefault(day, 0);
            days.put(day, calories + userMeal.getCalories());
        });

        List<UserMealWithExcess> listResult = new ArrayList<>();
        meals.forEach((userMeal) -> {
            if (isBetweenHalfOpen(userMeal.getDateTime().toLocalTime(), startTime, endTime)) {
                boolean excess = days.get(userMeal.getDateTime().toLocalDate()) > caloriesPerDay;
                listResult.add(new UserMealWithExcess(userMeal.getDateTime(), userMeal.getDescription(), userMeal.getCalories(), excess));
            }
        });
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        return meals.stream()
                ...
                .peek(userMeal -> sumCaloriesDay.put(
                        userMeal.getDateTime().toLocalDate(),
                        sumCaloriesDay.getOrDefault(userMeal.getDateTime().toLocalDate(),0) + userMeal.getCalories()))
